...
Qwen3-14B는 Thinking mode(추론 과정 출력)와 Non-thinking mode(즉시 답변)를 동일 모델에서 지원한다.  
본 실험에서는 두 가지 핵심 질문을 탐구한다.
...
| gradio | 6.11.0 | 채팅 데모 UI |
| matplotlib | 3.10.8 | Loss Curve, 평가 그래프 |
| numpy | 2.4.0 | 수치 계산 |
| pandas | 3.0.2 | 데이터 혼합 처리 |

> Apple Silicon(MPS) 환경에서 `chat_demo.ipynb`를 실행하려면 `mlx`, `mlx-lm`을 추가로 설치한다.  
> `requirements.txt` 하단 주석 처리된 항목을 참고.
...
**Thinking 억제 현상**  
KoAlpaca(75%)가 `<think>` 없는 일반 대화 데이터이므로, 파인튜닝 후 모든 질문에서 `<think>` 가 비어있는 현상이 관찰된다. 한국어 응답 품질은 향상됐으나 reasoning 능력이 억제됨. reasoning 비율을 높이거나 한국어 CoT 데이터를 추가하면 개선 가능하다.

**도메인 특화 한계**  
IT/ML 특화 질문(콜드 스타트 등)에서 Base 모델보다 답변이 단순화되는 경향이 관찰된다. KoAlpaca가 일반 도메인 데이터이기 때문으로, 도메인 특화 데이터로 교체하면 개선 가능하다.

**답변 구체성 저하**  
Base 모델이 마크다운 구조화, 수식 포함 등 더 상세한 답변을 생성하는 경우가 있다. 한국어 품질 향상과 답변 구체성 사이의 트레이드오프가 존재하며, 데이터 품질 개선으로 해소 가능하다.

**학습 규모**  
`max_steps=500`은 전체 에포크 학습 대비 소규모 실험 수준으로, 프로덕션 품질을 위해서는 1,000 스텝 이상이 권장된다.

### 11.2 후속 연구 방향

- 한국어 CoT 데이터셋 구축 또는 OpenMathReasoning 번역으로 thinking 억제 현상 해소
- reasoning 비율을 50:50으로 높여 thinking 능력 유지 실험
- 커머스/추천 도메인 특화 instruction 데이터로 교체하여 도메인 특화 파인튜닝
- `num_train_epochs=1` 전체 학습 후 성능 비교
- KMMLU, HAE-RAE 등 한국어 벤치마크로 정량 평가 추가
