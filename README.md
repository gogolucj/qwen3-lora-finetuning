...
- 단, Base 모델이 마크다운 구조화, 수식 포함 등 더 상세한 답변을 생성하는 경우가 있다.
...
이를 해소하려면 reasoning 데이터 비율을 높이거나 한국어 CoT 데이터를 추가 필요하다.    
...
**Thinking 억제 현상**  
KoAlpaca(75%)가 `<think>` 없는 일반 대화 데이터이므로, 파인튜닝 후 모든 질문에서 `<think>` 가 비어있는 현상이 관찰된다.      
한국어 응답 품질은 향상됐으나 reasoning 능력이 억제된다.  
reasoning 비율을 높이거나 한국어 CoT 데이터를 추가하면 개선 가능할 것으로 보인다.  

**도메인 특화 한계**  
IT/ML 특화 질문(콜드 스타트 등)에서 Base 모델보다 답변이 단순화되는 경향이 관찰된다.  
KoAlpaca가 일반 도메인 데이터이기 때문으로, 도메인 특화 데이터로 교체하면 개선 가능할 것으로 보인다.

**답변 길이 감소 및 구체성 저하**  
파인튜닝 후 응답 길이가 Thinking OFF 기준 평균 855자 → 341자(-60%), Thinking ON 기준 2,204자 → 509자(-77%)로 대폭 감소했다.  
KoAlpaca 데이터의 output이 전반적으로 짧아, 모델이 짧은 응답 패턴을 학습한 것으로 분석된다.  
마크다운 구조화, 수식 포함 등 상세한 서술도 줄어들었다. 고품질 장문 한국어 데이터를 추가하면 개선 가능할 것으로 보인다.
