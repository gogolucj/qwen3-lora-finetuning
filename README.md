...
|------|------------|------------|
| Base | 59.3% | 30.2% |
| Fine-tuned | **82.9%** | **83.6%** |

- **Thinking OFF**: 59.3% → 82.9% (+23.6%p 개선)
- **Thinking ON**: 30.2% → 83.6% (+53.4%p 개선)

Base 모델의 Thinking ON 최종 답변 한국어 비율이 30.2%로 낮은 이유는,  
Q2·Q4·Q5에서 `<think>` 태그 없이 최종 답변까지 영어로 출력되었기 때문이다.  
`<think>` 내부 추론 과정은 출력된 Q1·Q3 모두 영어였다 (10.4절 참고).
...
| Q2 1~100 합계 | thinking ON | `<think>` **태그 없음**, 영어 답변 | `<think>` **비어있음**, 한국어 답변 |
| Q3 콜드 스타트 | thinking OFF | 사용자/아이템 두 유형 정확하게 설명 | 정확하나 Base보다 간결 |
| Q3 콜드 스타트 | thinking ON | `<think>` 영어로 추론 후 한국어 답변 | `<think>` **비어있음**, 한국어 답변 |
| Q4 닭과 달걀 | thinking OFF | 과학적/철학적 관점 다각도 설명 | 달걀 먼저 결론 위주로 단순화 |
| Q4 닭과 달걀 | thinking ON | `<think>` **태그 없음**, 영어 답변 | `<think>` **비어있음**, 한국어 답변 |
| Q5 데이터 불균형 | thinking OFF | 오버/언더샘플링, 유형까지 상세 설명 | 재샘플링, 데이터 합성 방법 서술 |
| Q5 데이터 불균형 | thinking ON | `<think>` **태그 없음**, 영어 답변 | `<think>` **비어있음**, 한국어 답변 |

**관찰 요약**

- 한국어 응답 품질은 전반적으로 향상됨
- 파인튜닝 후 응답 길이가 크게 감소함 — Thinking OFF 평균 855자 → 341자 (-60%), Thinking ON 평균 2,204자 → 509자 (-77%)
- 단, Base 모델이 마크다운 구조화, 수식 포함 등 더 상세한 답변을 생성하는 경우가 있음
- 한국어 품질 향상과 답변 길이·구체성 사이의 트레이드오프 존재

### 10.4 Thinking 언어 관찰

| 질문 | Base `<think>` | Fine-tuned `<think>` |
|------|--------------|---------------------|
| Q1 전세 계약 | 영어로 추론 | **비어있음** |
| Q2 1~100 합계 | **태그 없음** (thinking 미작동) | **비어있음** |
| Q3 콜드 스타트 | 영어로 추론 | **비어있음** |
| Q4 닭과 달걀 | **태그 없음** (thinking 미작동) | **비어있음** |
| Q5 데이터 불균형 | **태그 없음** (thinking 미작동) | **비어있음** |

Base 모델은 Q1·Q3에서 영어로 추론하고, Q2·Q4·Q5에서는 `<think>` 태그 자체가 출력되지 않았다.  
Fine-tuned 모델은 모든 질문에서 `<think>` 가 비어있는 현상이 관찰된다.

**원인 분석**: KoAlpaca 데이터가 전부 `<think>` 없는 일반 대화 형식이며, 75% 비율로 학습되면서  
모델이 "한국어 질문 → 바로 답변" 패턴을 강하게 학습한 것으로 분석됨.  
파인튜닝이 한국어 응답 품질은 높였으나 thinking 능력을 억제하는 방향으로 작용함.   
이를 해소하려면 reasoning 데이터 비율을 높이거나 한국어 CoT 데이터를 추가 필요함.  
...
**Thinking 억제 현상**  
KoAlpaca(75%)가 `<think>` 없는 일반 대화 데이터이므로, 파인튜닝 후 모든 질문에서 `<think>` 가 비어있는 현상이 관찰됨.    
한국어 응답 품질은 향상됐으나 reasoning 능력이 억제됨.  
reasoning 비율을 높이거나 한국어 CoT 데이터를 추가하면 개선 가능할 것으로 보임.

**도메인 특화 한계**  
IT/ML 특화 질문(콜드 스타트 등)에서 Base 모델보다 답변이 단순화되는 경향이 관찰됨.  
KoAlpaca가 일반 도메인 데이터이기 때문으로, 도메인 특화 데이터로 교체하면 개선 가능할 것으로 보임.

**답변 길이 감소 및 구체성 저하**  
파인튜닝 후 응답 길이가 Thinking OFF 기준 평균 855자 → 341자(-60%), Thinking ON 기준 2,204자 → 509자(-77%)로 대폭 감소함.
KoAlpaca 데이터의 output이 전반적으로 짧아, 모델이 짧은 응답 패턴을 학습한 것으로 분석됨.  
마크다운 구조화, 수식 포함 등 상세한 서술도 줄어들었다. 고품질 장문 한국어 데이터를 추가하면 개선 가능할 것으로 보임.
